...
static int simple_init(void)
{
    ...
    h =HZ;
    /////////////////////
     printk(KERN_INFO "init_jiffies: %lu\n",js);//  印出載入時的jiffies，使用％lu來表示，格式為"init_jiffies: XXXXXXXXXXX"，記得換行
     printk(KERN_INFO "HZ: %d\n",h);  //印出系統定義的的HZ，使用％d來表示，格式為"HZ: XXX"，記得換行
       return 0;
}
...
static void simple_exit(void) {
    ...
    jz = jiffies;
    /////////////////////>
    printk(KERN_INFO "exit_jiffies: %lu\n",jz) ;  //印出移除時的jiffies，使用％lu來表示，格式為"exit_jiffies: XXXXXXXXXXX"，記得換行
